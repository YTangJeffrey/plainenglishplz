...
const toneDescriptions: Record<Exclude<AudienceTone, 'custom'>, string> = {
...
const describeAudience = (tone: AudienceTone, customGuide?: CustomGuide) => {
  if (tone === 'custom') {
    const name = customGuide?.name?.trim() || 'Custom Guide';
    const desc = customGuide?.description?.trim() || 'Describe the artwork in plain, approachable language.';
    return `${name}: ${desc}`;
  }
...
export const buildFollowUpUserPrompt = (
  tone: AudienceTone,
  customGuide: CustomGuide | undefined,
  labelText: string,
  history: ChatMessage[],
  question: string,
  explanation: string,
